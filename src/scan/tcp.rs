...
mod tests {
    ...
    fn test_send_null_scan_packet() {
        ...
        let timeout = Duration::from_secs(1);
        let max_loop = 8;
        let ret = send_null_scan_packet(src_ipv4, dst_ipv4, 49511, 80, timeout, max_loop);
        println!("{:?}", ret);
    }
    #[test]
    fn test_send_tcp_handshark() {
        ...
        let ret =
            send_connect_scan_packet(src_ipv4, dst_ipv4, src_port, 81, timeout, max_loop).unwrap();
        println!("{:?}", ret);
    }
}
