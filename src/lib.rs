...
impl PistolStream {
    ...
    fn run_receiver(
        ...
        for interface in datalink::interfaces() {
            ...
            thread::spawn(move || {
                ...
                let mut cap = match Capture::new(&interface.name) {
                    Ok(cap) => cap,
                    Err(e) => {
                ...
                loop {
                    ...
                    if let Ok(packets) = cap.fetch_as_vec() {
                        ...
                        for p in packets {
                            // #[cfg(feature = "debug")]
                            // debug_show_packet(p, Some(EtherTypes::Ipv4));
                            pa.push(Arc::from(p));
                        }
                        if let Err(e) = sender.send(pa) {
                            // timeout will drop sender
                            debug!("send packet to {} error: {}", interface.name, e);
                        }
                    }
                }
            });
        ...
        if all_ready {
            ...
            panic!("some receivers failed to start");
        }
    }
    /// Initialize the PistolStream,
    /// which includes initializing the layer2 sender,
    /// layer3 sender and receiver.
    fn init(&mut self, filter: Option<String>) -> Result<(), PistolError> {
        ...
        match Self::run_receiver(sender, filter) {
            Ok(count) => {
    ...
    fn send_packet(&mut self, input: SendPacketParam) -> Result<(), PistolError> {
        loop {
            ...
            if self.l2_receiver_ready {
                ...
                if !self.l2_receiver_ready_sleep {
                    ...
                    let mut sleep_millis = self.l2_receiver_count * 100;
                    if sleep_millis > 500 {
        ...
        let eth_type = input.eth_type;
        let if_name = input.if_name;
        let retransmit = input.retransmit;

        let payload_len = l3_payload.len();
        ...
        if dst_mac == src_mac {
            ...
            for (ifn, l3s) in &mut self.l3_senders {
                ...
                debug!("send loopback packet through {}: {}", ifn, m);
                let mut l3_payload = l3_payload.to_vec();
                match eth_type {
                    EtherTypes::Ipv4 => {
                        let mut packet = MutableIpv4Packet::new(&mut l3_payload).ok_or(
                        ...
                        for _ in 0..retransmit {
                            l3s.sender_ipv4
                                .send_to(&packet, Ipv4Addr::LOCALHOST.into())?;
                        }
                    }
                    EtherTypes::Ipv6 => {
                        let mut packet = MutableIpv6Packet::new(&mut l3_payload).ok_or(
                            PistolError::BuildPacketError {
                                location: Location::caller().to_string(),
                            },
                        )?;
                        packet.set_source(Ipv6Addr::LOCALHOST);
                        packet.set_destination(Ipv6Addr::LOCALHOST);
                        for _ in 0..retransmit {
                            l3s.sender_ipv6
                                .send_to(&packet, Ipv6Addr::LOCALHOST.into())?;
                        }
                    }
                    _ => {
                        error!(
...
impl Pistol {
    ...
    pub fn set_log_level(&mut self, log_level: &str) {
        let log_level = match log_level.to_lowercase().as_str() {
            ...
            "none" => None,
            _ => {
                warn!("unknown log_level: {}, set it to [info] now", log_level);
                Some(Level::INFO)
            }
        };
    ...
    pub fn icmp_address_mask_ping_raw(
        ...
        self.init_logger();
        let (ping_target, dur) = PingTargetWithNetInfo::infer_icmp_single(dst_addr, src_addr)?;
...
impl XxpPingTarget {
    ...
    /// If possible, convert the domain name to an IPv6 address, otherwise return an error
    pub fn from_domain6(
        dst_domain: &str,
        dst_ports: Vec<u16>,
        src_addr: Option<IpAddr>,
...
#[derive(Debug, Clone)]
pub(crate) struct PingTargetWithNetInfo {
    pub net_info: NetInfo,
    /// For ICMP ping, the destination port is not used, so it is set to None. For TCP ping, it is used if specified.
    pub dst_ports: Vec<u16>,
