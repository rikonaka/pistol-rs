...
fn send_seq_probes(
    ...
    stream.init(Some(format!("host {} and tcp", dst_ipv6)))?;
...
fn send_u1_probe(
    ...
    let mut stream = PistolStream::new();
    stream.init(Some(format!("host {} and (icmp6 or udp)", dst_ipv6)))?;
